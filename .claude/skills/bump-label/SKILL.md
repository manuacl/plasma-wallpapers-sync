...
- Invoked from a wrapper skill like `finish-branch` right after
  `gh pr create`.
...
| Only `docs/`, `README.md`, `CLAUDE.md`, `.claude/`, `.github/` touched | `none` |
...
if echo "$commits" | grep -qE '(BREAKING CHANGE|^[a-z]+(\([^)]+\))?!:)'; then
    ...
    recommended=minor
elif ! echo "$changed" | grep -qvE '^(docs/|README\.md|CLAUDE\.md|\.claude/|\.github/)'; then
    recommended=none
...
# Always strip any pre-existing bump:* label first (a PR has at most
# one, and switching from major→minor on a re-run must not stack).
...
if [ "$chosen" = "none" ]; then
    ...
    # Ensure the three bump labels exist on the repo (idempotent —
    # fails silently if already present). bump:none is NOT a label —
...
The label is consumed by `.github/workflows/version.yml` (bundled with
this repo): on push to main, it finds the merged PR, reads its
`bump:*` label, bumps the project's `VERSION` field inside
`CMakeLists.txt` via `sed`, commits the change and tags `vX.Y.Z`. With
no `bump:*` label, the workflow exits cleanly — matching this skill's
"no label = no bump" default.

When copying this skill into a project with a different version source
(metadata.json, package.json, …), only `version.yml`'s bump step
changes — this skill stays the same.
